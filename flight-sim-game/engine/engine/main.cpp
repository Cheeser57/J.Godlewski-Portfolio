...
int main()
{
    ...
    //plane = PhysObject(&res.root, planeModel2, -1, glm::vec3(-200, 0, -8200), glm::vec3(0.01), glm::vec3(0, 0, 0));

    Generate_Sky_Ground(&res, glm::vec3(0, -10, 0), 80, glm::vec3(0, 0, 0), 8);
    Generate_Airport(&res, glm::vec3(0, 0, -8450), 2, glm::vec3(0, 0, 0));
    Generate_City(&res);

    GameObject floor = GameObject(&res.root);
    GameObject lamp = GameObject(&res.root,-1,-1,glm::vec3(-510,150,-2073));
    ...
    while(!glfwWindowShouldClose(gm.window))
    {
        while(!glfwWindowShouldClose(gm.window))
        {
            ...
            if (Collision(&plane))
            {
                ...
                res.drawScene(gm.camera, gm.perspective, gm.uiProjection, gm.lightPos, lamp.pos);
            ...
            std::cout << plane.wingTipLeft().x << " " << plane.wingTipLeft().y << " " << plane.wingTipLeft().z << std::endl;
            std::cout << plane.pos.x << " " << plane.pos.y<<" " << plane.pos.z << std::endl << std::endl;
