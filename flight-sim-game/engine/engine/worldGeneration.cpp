...
void Generate_Airport(ResourceManager* res, glm::vec3 position, float size, glm::vec3 rotation)
{
    ...
    for (int i = -1; i < 2; i++)
    {
        GameObject* street = new GameObject(&res->root, streetModel, streetTex, glm::vec3(3.45 + 100 * i, -2, 100) * size + position, glm::vec3(2, 2, 6) * size, glm::vec3(0, PI / 2, 0) + rotation);
        street->isReflective = 0.0f;
        res->renderQueue.push_back(street);
        GameObject* street2 = new GameObject(&res->root, streetModel, streetTex, glm::vec3(3.45 + 100 * i, -2, 100 + 63.5) * size + position, glm::vec3(2, 2, 6) * size, glm::vec3(0, PI / 2, 0) + rotation);
        street2->isReflective = 0.0f;
        res->renderQueue.push_back(street2);
        GameObject* street3 = new GameObject(&res->root, streetModel, streetTex, glm::vec3(3.45 + 100 * i, -2, 100 + 2 * 63.5) * size + position, glm::vec3(2, 2, 6) * size, glm::vec3(0, PI / 2, 0) + rotation);
        street3->isReflective = 0.0f;
        res->renderQueue.push_back(street3);
        GameObject* street4 = new GameObject(&res->root, streetModel, streetTex, glm::vec3(3.45 + 100 * i, -2, 100 + 3 * 63.5) * size + position, glm::vec3(2, 2, 6) * size, glm::vec3(0, PI / 2, 0) + rotation);
        street4->isReflective = 0.0f;
        res->renderQueue.push_back(street4);
        GameObject* street5 = new GameObject(&res->root, streetModel, streetTex, glm::vec3(3.45 + 100 * i, -2, 100 + 4 * 63.5) * size + position, glm::vec3(2, 2, 6) * size, glm::vec3(0, PI / 2, 0) + rotation);
        street5->isReflective = 0.0f;
...
void Generate_City(ResourceManager* res)
{
    ...
    int city_lampModel = res->loadModel("Street Lamp.obj");
    ...
    int bricksTex = res->addTexture("bricks.png");///bricks
    GameObject* city_lamp = new GameObject(&res->root, city_lampModel, bricksTex, glm::vec3(-510, 48, -2073), glm::vec3(0.05), glm::vec3(0, 0, 0));
    res->renderQueue.push_back(city_lamp);
}
